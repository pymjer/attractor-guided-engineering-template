...
**Changes**:
- 2026-08-13 (prompt-contract-optimization): **Option B — review-gate ownership restored to REVIEW_PLANS.** The draft prompts (`draft-from-roadmap.md`, `draft-from-audit.md`) no longer spawn an internal review sub-agent or self-promote to `active`; they self-check and leave the plan at `draft`. `REVIEW_PLANS` (an independent agent-session `forEach: draftPlans()` step running `plan-review.md`) is the mandatory rule-13 independent review gate and the only step that flips `draft → active` (fix-forward; genuine unresolved Blocker → `Review Hold`). Rationale: engine-structural independence (reviewer session ≠ drafter session) instead of prompt-wording independence, eliminating the E5 bypass. Also: **roadmap item ↔ plan 1:1** (`draft-from-roadmap.md`; large item → multiple Phases in one plan, never bundle items; enforced by `plan-review.md` via the `> Work Item:` field — audit-sourced plans are exempt). Honest-failure edges added: `DRAFT_PLANS`, and the deep-audit-loop steps (`CHECK_OPEN_AUDITS`/`MULTI_AUDIT`/`OPEN_AUDIT`/`SCAN_NEW_RESULTS`) each accept a `fail` marker → `done:"failed"`; `plan-execution` `CLOSURE_AUDIT.onMaxRetries` changed from `goto: BUILD_VERIFY` to `done:"failed"` (non-convergent closure is now honest, not laundered into a pass). State-ownership: `EXECUTE` no longer flips roadmap/audit/plan-completed; those flips belong solely to `BUILD_VERIFY` after closure. Audit prompts emit tri-state headers `clean`/`triaged`/`open`. `prompt-check.mjs` extended (marker existence + `created⇒FLOW_VARS/PLAN_FILE` ordering + `issues⇒REMAINING` + forEach value membership + injected-var allowlist + project-leakage scan).
...
5. **Draft self-checks; REVIEW_PLANS reviews (Option B, 2026-08-13)**: `DRAFT_PLANS` writes plans and self-checks only, leaving them at `draft`. The mandatory independent review is `REVIEW_PLANS` — an independent agent-session `forEach: draftPlans()` step (`plan-review.md`) that fix-forward-revises and is the sole promoter to `active`. (This supersedes the v5 "DRAFT merges review into one step" design, which relied on prompt-wording independence and could bypass the formal gate.)
...
- Writes plans from remaining roadmap items and **self-checks only** (Option B, 2026-08-13): no internal review sub-agent, no self-promotion. Each plan is left at `> Plan Status: draft` for `REVIEW_PLANS`.
- **Roadmap item ↔ plan 1:1**: exactly one plan per roadmap item; a large item is split into multiple Phases inside one plan, never bundled across items. The plan records the single item in its `> Work Item:` field.
- **Exits (post-WI4, mdc-1, Option B)**: `created` → REVIEW_PLANS; `nothing` → DEEP_AUDIT, OR audit-gate short-circuit to `completed` when `auditRound >= 1 && openAudits().length === 0 && activePlans().length === 0` (mdc-1 clean short-circuit — engine decision, not AI; `maxAuditRounds` remains the safety ceiling). `fail` → `done:"failed"` (honest termination on unreadable/contradictory roadmap; added 2026-08-13). The legacy `done` exit was removed in WI4. `onMaxRetries` is `done:"failed"`.

**REVIEW_PLANS** (`prompts/plan-review.md`, `forEach: draftPlans()`)
- Independent agent-session step and the **mandatory rule-13 independent review gate** (reviewer session ≠ drafter session). On a fresh start / no drafts, `draftPlans() → []` and the engine short-circuits the empty forEach to `all_complete` with **zero AI calls**.
- Fix-forward: resolves Blocker/Major in place and flips `draft → active`; a genuine unresolved Blocker stays `draft` with `> Review Hold:`. Per-item `approved` marker aliases to the aggregate `all_complete`.
- Enforces roadmap 1:1: a roadmap-sourced plan (has `> Work Item:`) bundling multiple items is a Blocker; audit-sourced plans (have `> Source Audits:`, no `> Work Item:`) are exempt.
