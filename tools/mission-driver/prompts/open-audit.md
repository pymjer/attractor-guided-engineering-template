...
Perform an open-ended adversarial audit on mission `{{missionName}}`, probing `{{moduleDir}}/` for contract drift, dead code, missing error handling, framework anti-patterns, and `AGENTS.md` convention violations (read `AGENTS.md` completely). Sample by risk toward the areas `{{openAuditPrompt}}` names; you need not read every file.
...
- **`[P2]`** — trivial/non-blocking polish: doc rot, wording, naming, dead-comment nits. Recorded but does not by itself warrant a plan.
...
Write to `{{auditsDir}}/{{TIMESTAMP}}-open-audit-{{missionName}}.md`. The header's `Audit Status` MUST reflect the real outcome:
